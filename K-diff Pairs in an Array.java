/*
Given an array of integers and an integer k, you need to find the number of unique k-diff pairs in the array. Here a k-diff pair is 
defined as an integer pair (i, j), where i and j are both numbers in the array and their absolute difference is k.

Example 1:
Input: [3, 1, 4, 1, 5], k = 2
Output: 2
Explanation: There are two 2-diff pairs in the array, (1, 3) and (3, 5).
Although we have two 1s in the input, we should only return the number of unique pairs.
Example 2:
Input:[1, 2, 3, 4, 5], k = 1
Output: 4
Explanation: There are four 1-diff pairs in the array, (1, 2), (2, 3), (3, 4) and (4, 5).
Example 3:
Input: [1, 3, 1, 5, 4], k = 0
Output: 1
Explanation: There is one 0-diff pair in the array, (1, 1).
Note:
The pairs (i, j) and (j, i) count as the same pair.
The length of the array won't exceed 10,000.
All the integers in the given input belong to the range: [-1e7, 1e7].
*/


// Passed on LeetCode: Beats 78% of Submissions
// Using HashMap to keep the count of the elements in the array
// T: O(n)

class Solution {
    public int findPairs(int[] nums, int k) {
        
        if(nums == null || nums.length == 0 || k < 0)   return 0;
        
        HashMap<Integer, Integer> result = new HashMap<Integer, Integer>();
        int count = 0;    
        
        for(int n: nums){
            
            if(result.containsKey(n)){
                
                if(k == 0 && result.get(n) == 1){
                    count++;
                }
                
                result.put(n, result.getOrDefault(n, 0) + 1);
            }else{
                
                if(result.containsKey(n - k)){
                    count++;
                }
                if(result.containsKey(n + k)){
                    count++;
                }
                
                result.put(n, 1);
            }
        }
        
        return count;
    }
}


// There are several other ways of doing this.
// 1. Using Brute Force - Find all posibile values - O(n ^ 2)
// 2. Using self-blancing BST - AVL Tree - O(n log n)
// 3. Using Sorting and keep two pointers to keep track - O(n log n)

class Solution {
    public int findPairs(int[] nums, int k) {
        if (k < 0) {
            return 0;
        }
        
        Arrays.sort(nums);
        
        int counter = 0;
        
        int left = 0;
        int right = 1;
        
        HashSet<Integer> set = new HashSet<>();
        
        while (right < nums.length) {
            int s = nums[right] - nums[left];
            
            if (s == k && left != right && set.add(nums[right])) {
                counter++;               
                left++;
                right++;
            } else if (s > k) {
                left++;
            } else {
                right++;
            }
        }
        
        return counter;
    }
}